...
func (_q *CertificationQuery) QueryResume() *ResumeQuery {
...
func (_q *CertificationQuery) WithResume(opts ...func(*ResumeQuery)) *CertificationQuery {
	query := (&ResumeClient{config: _q.config}).Query()
...
//		GroupBy(certification.FieldName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Certification.Query().
//		Select(certification.FieldName).
//		Scan(ctx, &v)
...
func (_q *CertificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Certification, error) {
	...
	if query := _q.withResume; query != nil {
		if err := _q.loadResume(ctx, query, nodes, nil,
			func(n *Certification, e *Resume) { n.Edges.Resume = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CertificationQuery) loadResume(ctx context.Context, query *ResumeQuery, nodes []*Certification, init func(*Certification), assign func(*Certification, *Resume)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Certification)
	for i := range nodes {
		if nodes[i].resume_certifications == nil {
			continue
		}
		fk := *nodes[i].resume_certifications
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "resume_certifications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
